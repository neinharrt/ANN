...
bool NeuralNetwork::Init(const int &molecule_flag, const int &mode_index, const std::string &species_name)
{
  ...
  fin >> x1_log_ >> x2_log_ >> x3_log_ >> x4_log_ >> y_log_;
...
void NeuralNetwork::Pred(const double *x, double *f) const
{
  *f = bo_[0];
  for (int i = 0; i < m; i++)
  {
    const double yi = Ai_[i * 4 + 0] * x[0] + Ai_[i * 4 + 1] * x[1] + Ai_[i * 4 + 2] * x[2] + Ai_[i * 4 + 3] * x[3] + bi_[i];
    *f += (Ao_[i] * Transfer(yi));
  }
}
void NeuralNetwork::Derivative(const double *x, double *dfdx) const
{
  ...
  for (int i = 0; i < m; i++)
  {
    ...
    dfdx[0] += (temp * Ai_[i * 4 + 0]);
    dfdx[1] += (temp * Ai_[i * 4 + 1]);
    dfdx[2] += (temp * Ai_[i * 4 + 2]);
